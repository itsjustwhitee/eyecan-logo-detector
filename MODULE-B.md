...
A **sample pool of ~1000 background images** is included in `sample_background/` for quick tests and reproducibility without downloading the full dataset.
...
```bash
# All defaults (5000 samples, 640×480, backgrounds/Images)
python src/dataset_generator.py

# Quick test using the bundled sample pool
python src/dataset_generator.py \
...
|---|---|---|
| `--bg_dir` | `backgrounds/Images` | Background images directory (recursive). Use `sample_background` for quick tests. |
...
**50 % of the shorter canvas dimension** (`max_logo_ratio=0.5`), keeping
the logo clearly visible in the scene.
...
|---|---|---|
| `Affine` | scale 0.025–0.85, rotate ±180°, translate ±25%, shear ±10°, p=0.99 | Full size, orientation, position and shear diversity. |
| `Perspective` | warp 5–10%, p=0.5 | Logo seen from an angle. |
...
|---|---|---|
| `HueSaturationValue` | hue ±8, sat ±20, val ±15, p=0.65 | Simulates different print materials and coloured lighting on the logo surface. |
| `RGBShift` | ±10 per channel, p=0.30 | Subtle independent per-channel offset for further colour diversity. |
...
| Transform | Parameters | Effect |
|---|---|---|
| `RandomBrightnessContrast` | ±35% brightness, ±25% contrast, p=0.70 | Dark scenes, overexposed scenes, midtone variation. |
| `GaussianBlur` | kernel 3–8, p=0.15 | Defocus, motion blur, surveillance-camera quality. |
| `GaussNoise` | var 1.0–7.0, p=0.10 | Sensor noise, low-light grain. |
| `ImageCompression` | quality 70–95, p=0.20 | JPEG artefacts common in real photos and video frames. |
...
writing would require 10+ GB. pipelime `Item` objects keep an uncompressed
copy of the pixel data until they are serialised to disk.
...
3. Rename the files into the main `data/` folder with the correct global index offset.
4. Drop the Python list (`batch = []`) — all `Sample` objects are garbage-collected and RAM is freed before the next batch begins.
...
generate_dataset(
    bg_dir      = Path("sample_background"),   # bundled pool for quick tests
...
Use `bg_dir = Path("backgrounds/Images")` when working with the full MIT Indoor Scenes dataset.
