...
Displays each prediction in an interactive GUI window with a crosshair,
...
def main() -> None:
    ...
    # Force CPU device for WSL compatibility based on previous hardware checks
    ...
    for img_path in image_paths:
        ...
        # Draw crosshair marker (Red: BGR 0, 0, 255)
        # Radius scales dynamically based on image resolution
        ...
        # GUI Display
        # Using WINDOW_NORMAL to allow resizing, as modern phone photos exceed monitor resolution
        window_name = f"Prediction: {img_path.name} - PRESS ANY KEY TO CONTINUE"
        cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
        cv2.resizeWindow(window_name, 800, 600)
        cv2.imshow(window_name, bgr)
        
        # Wait for user to press any key before closing the window and moving to the next image
        cv2.waitKey(0)
